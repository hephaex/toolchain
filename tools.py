...
flags.DEFINE_string(
    'hub_repository', None,
    'Push tags to this Docker Hub repository, e.g. lablup/backend.ai-kernels')
...
flags.DEFINE_string(
    ...
    ('Regular expression that skips processing on any tag it matches. Must '
     'match entire string, e.g. ".*cuda.*" ignores all GPU tags.'),
    short_name='x')
...
flags.DEFINE_string(
    ...
    ('Regular expression that skips processing on any tag it does not match. '
     'Must match entire string, e.g. ".*cuda.*" includes only GPU tags.'),
    short_name='i')
...
class DockerTagValidator(cerberus.Validator):
  """Custom Cerberus validator for docker tag spec.
